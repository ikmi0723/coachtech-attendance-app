...
class AdminAttendanceUpdateRequest extends FormRequest
{
    ...
    public function withValidator($validator): void
    {
        $validator->after(function ($validator) {
            ...
            if ($this->isValidTime($clockIn) && $this->isValidTime($clockOut) && $clockIn >= $clockOut) {
            ...
            foreach ($breaks as $index => $break) {
                ...
                if (
                    ($breakStart && !$this->isValidTime($breakStart)) ||
                    ($breakEnd && !$this->isValidTime($breakEnd))
                ) {
                    continue;
                }
    ...
    public function messages(): array
    {
        return [
            ...
            'clock_in.date_format' => '出勤時間を正しい形式で入力してください',
            'clock_out.date_format' => '退勤時間を正しい形式で入力してください',
            'breaks.*.break_start_at.date_format' => '休憩時間を正しい形式で入力してください',
